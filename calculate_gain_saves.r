...
diffgroup_data <- as.data.table(read.csv('./data/data_for_gain_calculation/main_diffgroup_data_saves.csv'))
agentwise_data <- read.csv('./data/data_for_gain_calculation/agentwise_data_saves.csv')
callerwise_data <- read.csv('./data/data_for_gain_calculation/callerwise_data_saves.csv')
...
calculateAPCorrection <- function(agentwise_data,ncalls.matrix.agentwise,aq_100){
  ...
  return((sum(tmp$ncalls.x*tmp$save_rate)/sum(tmp$ncalls.x)-aq_100)/aq_100)
}
calculateCPCorrection <- function(callerwise_data,ncalls.matrix.callerwise,cq_100){
  ...
  return((sum(tmp$ncalls.x*tmp$save_rate)/sum(tmp$ncalls.x)-cq_100)/cq_100)
}
...
calculateGainByDiffgroup <- function(main_diffgroup_data,agentwise_data,callerwise_data,on_off_preference='all'){
  ...
  aq_100 <- sum(agentwise_data$nsaves)/sum(agentwise_data$ncalls)
  cq_100 <- sum(callerwise_data$nsaves)/sum(callerwise_data$ncalls)
  gains <- c()
  ap_corrections <- c()
  cp_corrections <- c()
  adjusted_gains <- c()
...
write.csv(gain_by_diffgroup_all,'./data/data_for_gain_calculation/gain_by_diffgroup_saves.csv',row.names = FALSE)
